...
import random
from crear_usuario import iniciar_sesion,registro_usuario
...
def ventana_usuario(arUser):
    ...
    # ---- Texto para el Entry -- 
    nombre_label = Label(login,text=TEXTO_USUARIO)
    nombre_label.grid(column=0,row=1,sticky="w")

    clave_label = Label(login,text=TEXTO_CLAVE)
    clave_label.grid(column=0,row=2,sticky="w")

    Label(ventana,text=TEXTO_RESTRICCION,fg="red").pack()
    Label(ventana,text=TEXTO_USUARIOS_CONECTADOS).pack()
    ...
    #---- Button del frame  ---
    boton_visual = Button(login,text=TEXTO_VER,command=lambda: ver_ocultar_clave())
    ...
    boton_configuracion = Button(login,text=TEXTO_CONFIG,command=lambda: ventana_configuracion())
...
def ventana_registro(arUser):
    ...
    # Label 
    crear_usuario_label = Label(mi_frame,text=TEXTO_NOMBRE)
    crear_usuario_label.grid(column=0,row=1,sticky="e")

    crear_clave_label = Label(mi_frame,text=TEXTO_CLAVE)
    crear_clave_label.grid(column=0,row=2,sticky="e")

    repetir_clave = Label(mi_frame,text=TEXTO_REPETIR_CLAVE)
    ...
    # Boton 
    boton_acceder = Button(ventana_secundaria,text=TEXTO_ACCESO,command=lambda: registro_usuario(arUser,cuadro_usuario.get(),cuadro_clave.get(),cuadro_repetir_clave.get()))
    boton_acceder.pack()

    ventana_secundaria.mainloop()

def ventana_configuracion():
    dicc = designar_configuracion()
    LONGITUD_PALABRA_MINIMA = dicc["LONGITUD_PALABRA_MINIMA"] 
    CANTIDAD_LETRAS_ROSCO = dicc["CANTIDAD_LETRAS_ROSCO"]
    MAXIMO_PARTIDAS = dicc["MAXIMO_PARTIDAS"]
    PUNTAJE_ACIERTO = dicc["PUNTAJE_ACIERTO"]
    PUNTAJE_DESACIERTO = dicc["PUNTAJE_DESACIERTO"]

    TAMANIO_VENTANA_CONFIG = "350x150"
    TEXTO_PALABRA_MINIMA = "Palabra minima:"
    TEXTO_LETRAS_ROSCO = "Letras en el rosco:"
    TEXTO_MAXIMO_PARTIDAS = "Maximo de partidas:"
    TEXTO_PUNTAJE_ACIERTO = "Puntaje acierto:"
    TEXTO_PUNTAJE_DESACIERTO = "Puntaje desacierto:"
    TEXTO_RESTRICCION = "Recuerda poner solo valores mayores o iguales a los que aparece"
    ...
    # Texto
    palabra_minima = Label(frame_config,text=TEXTO_PALABRA_MINIMA)
    palabra_minima.grid(column=0,row=0,sticky="e")

    letras_rosco = Label(frame_config,text=TEXTO_LETRAS_ROSCO) 
    letras_rosco.grid(column=0,row=1,sticky="e")

    max_partidas = Label(frame_config,text=TEXTO_MAXIMO_PARTIDAS)
    max_partidas.grid(row=2,column=0,sticky="e")

    puntaje_positivo = Label(frame_config,text=TEXTO_PUNTAJE_ACIERTO)
    puntaje_positivo.grid(row=3,column=0,sticky="e")

    puntaje_negativo = Label(frame_config,text=TEXTO_PUNTAJE_DESACIERTO)
    ...
    # Boton 
    boton_config = Button(ventana,text=TEXTO_BOTON_ACEPTAR)
    boton_config.pack()
...
def ventana_turnos(arUser):
    ...
    titulo = Label(ventana_terciaria,text=TEXTO_TURNO,font=("Arial",10))
    titulo.pack()
    contador = 1
